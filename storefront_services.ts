...
export type StorefrontServices = {
  ...
  orderService: { createFromCheckout: (payload: Record<string, unknown>) => Promise<{ id: string }> }
}
...
function normalizeColours(value: unknown): Array<{ name: string; hex?: string }> | undefined {
  ...
  const colours = value.map((colour) => {
...
function normalizeGallery(value: unknown, fallbackImage: string, productName: string): EdgeGalleryImage[] {
  if (!Array.isArray(value)) {
    return fallbackImage ? [{ index: 0, src: fallbackImage, alt: `${productName} at Adonai Thrift Store` }] : []
  }
...
export function toEdgeProduct(product: CatalogProduct): EdgeProduct {
  ...
  const originalPrice = product.originalPriceFormatted || product.compareAtPriceFormatted ||
    (product.originalPrice || product.compareAtPrice ? formatUgx(product.originalPrice || product.compareAtPrice) : undefined)
  ...
  return {
    ...
    sizes: Array.isArray(product.sizes) ? product.sizes.map((size) => asString(size)).filter(Boolean) : undefined,
    colours,
  }
}
...
class FlaskStorefrontGateway {
  ...
  async json<T extends FlaskResponse | CatalogProduct[] = FlaskResponse>(path: string, options: RequestInit = {}): Promise<T> {
    ...
    if (!response.ok) {
      const message = typeof parsed === 'object' && parsed && 'message' in parsed ? String(parsed.message) : `Flask storefront returned ${response.status}`
  ...
  return {
    ...
    catalogService: {
      ...
      async findAvailableProduct(id) {
        try {
          const data = await gateway.json<FlaskResponse | CatalogProduct>(`/api/products/${encodeURIComponent(id)}`)
    ...
    accountService: {
      signIn: (identifier, password, remember) => gateway.json('/api/account/sign-in', {
        method: 'POST',
        body: JSON.stringify({ identifier, password, remember }),
      }),
      register: (payload) => gateway.json('/api/account/sign-up', {
        method: 'POST',
        body: JSON.stringify(payload),
      }),
    },
    contactService: {
      send: (payload) => gateway.json('/api/contact', { method: 'POST', body: JSON.stringify(payload) }),
    },
    orderService: {
      async createFromCheckout(payload) {
        const data = await gateway.json('/api/orders', { method: 'POST', body: JSON.stringify(payload) })
        const id = data.order?.id || data.id
        if (id === undefined || id === null) throw new Error('The order service did not return an order id')
        return { id: String(id) }
      },
    },
  }
