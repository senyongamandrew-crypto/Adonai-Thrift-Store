...
const phoneField = () => vine.string().trim().maxLength(30).regex(phoneRule).transform((value: string) => value.replace(/[()\s.-]/g, ''))
...
export const signInValidator = vine.compile(vine.object({
  identifier: vine.string().trim().minLength(3).maxLength(180).regex(identifierRule).transform((value: string) => value.includes('@') ? value.toLowerCase() : value.replace(/[()\s.-]/g, '')),
...
export const checkoutValidator = vine.compile(vine.object({
  name: vine.string().trim().minLength(2).maxLength(120),
...
export const driverAssignmentValidator = vine.compile(vine.object({
