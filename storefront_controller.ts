...
export default class StorefrontController {
  ...
  async home(ctx: HttpContext) {
    ...
    const searchQuery = String(request.input('q') || '').trim().slice(0, 120)
    ...
    return view.render('pages/home', {
      ...
      metaDescription: 'Handpicked Grade-A thrift and vintage clothing in Kampala, with clear UGX prices and local delivery.',
      products: products.map(storefront.catalogService.toEdgeProduct),
      searchQuery,
      bagCount: 0,
    })
  }
  ...
  async product(ctx: HttpContext) {
    ...
    const product = await storefront.catalogService.findAvailableProduct(String(params.id || ''))
    if (!product) {
      response.status(404)
      return view.render('errors/not-found', { ...this.sharedViewData(request), pageTitle: 'Product not found · Adonai Thrift Store', noIndex: true })
    }
    ...
    return view.render('pages/product', {
      ...
      metaDescription: mappedProduct.description || `View ${mappedProduct.name} at Adonai Thrift Store.`,
      ogType: 'product',
  ...
  async auth(ctx: HttpContext) {
    ...
    return view.render('pages/auth', {
      ...
      pageTitle: isSignup ? 'Create an account · Adonai Thrift Store' : 'Sign in · Adonai Thrift Store',
    })
  }

  async contactPage(ctx: HttpContext) {
    const { request, view } = ctx as StorefrontHttpContext
    return view.render('pages/contact', { ...this.sharedViewData(request), pageTitle: 'Contact Adonai Thrift Store' })
  }

  async checkoutPage(ctx: HttpContext) {
    const { request, view } = ctx as StorefrontHttpContext
    return view.render('pages/checkout', { ...this.sharedViewData(request), pageTitle: 'Checkout · Adonai Thrift Store' })
  }
  ...
  async signIn(ctx: HttpContext) {
    ...
    await storefront.accountService.signIn(payload.identifier, payload.password, Boolean(payload.remember))
  ...
  async privacy(ctx: HttpContext) {
    const { request, view } = ctx as StorefrontHttpContext
    return view.render('pages/privacy', { ...this.sharedViewData(request), pageTitle: 'Privacy policy · Adonai Thrift Store' })
  }

  async terms(ctx: HttpContext) {
    const { request, view } = ctx as StorefrontHttpContext
    return view.render('pages/terms', { ...this.sharedViewData(request), pageTitle: 'Terms & conditions · Adonai Thrift Store' })
  }

  async notFound(ctx: HttpContext) {
    const { request, response, view } = ctx as StorefrontHttpContext
    response.status(404)
    return view.render('errors/not-found', { ...this.sharedViewData(request), pageTitle: 'Page not found · Adonai Thrift Store', noIndex: true })
  }
  ...
  async sitemap(ctx: HttpContext) {
    ...
    try {
      const products = await storefront.catalogService.availableProducts({ searchQuery: '' })
      productRoutes = products.map((product) => `/products/${encodeURIComponent(String(product.id || product.sku || ''))}`).filter((path) => !path.endsWith('/'))
    } catch {
      // Keep the legal/home sitemap available during a catalog outage.
    }
    const allRoutes = [...routes, ...productRoutes]
    const urls = allRoutes.map((path) => `<url><loc>${this.xmlEscape(`${siteUrl}${path}`)}</loc><changefreq>daily</changefreq><priority>${path === '/' ? '1.0' : '0.6'}</priority></url>`).join('')
    response.header('Content-Type', 'application/xml; charset=utf-8')
    return response.send(`<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${urls}</urlset>`)
  }
